...
def perform_metacc_leiden_clustering(scores, contig_len, min_binsize, num_gene, random_seed):
...
def evaluate_clustering(true_labels, predicted_labels, contig_len, bin_size_threshold):
    valid_bins = contig_len >= bin_size_threshold
    true_labels_filtered = true_labels[valid_bins]
    predicted_labels_filtered = np.array(predicted_labels)[valid_bins]
...
def main():
    ...
    norm_contact_matrix_with_self_loops = norm_contact_matrix
    mask = norm_contact_matrix.row != norm_contact_matrix.col
    norm_contact_matrix_without_self_loops = coo_matrix((norm_contact_matrix.data[mask], 
                                                        (norm_contact_matrix.row[mask], norm_contact_matrix.col[mask])), 
    ...
    for metric_name, metric_function in similarity_metrics.items():
        for include_self_loops in [True, False]:
            if include_self_loops:
                contact_matrix_to_use = norm_contact_matrix_with_self_loops
            else:
                contact_matrix_to_use = norm_contact_matrix_without_self_loops
            print(f"Calculating {metric_name} scores with {'self-loops' if include_self_loops else 'no self-loops'}")
            scores = calculate_similarity_scores(contact_matrix_to_use, metric_function)
            cutoff = get_cutoff(scores, 5)
            filtered_scores = filter_scores_by_cutoff(scores, cutoff)
            predicted_labels = perform_metacc_leiden_clustering(filtered_scores, contig_len, min_binsize, num_gene, random_seed)
            ari, nmi = evaluate_clustering(true_labels, predicted_labels, contig_len, bin_size_threshold)
            results.append({
                'Metric': metric_name,
