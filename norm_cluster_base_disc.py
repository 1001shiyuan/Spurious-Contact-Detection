...
# Discard lowest x% of normalized Hi-C contacts
def discard_lowest_percent(contact_matrix, percentile):
    threshold = np.percentile(contact_matrix.data, percentile)
    mask = contact_matrix.data > threshold
    return coo_matrix((contact_matrix.data[mask], (contact_matrix.row[mask], contact_matrix.col[mask])),
...
def main():
    ...
    norm_contact_matrix_without_self_loops = coo_matrix((contact_matrix.data[mask], 
                                                       ...
                                                       shape=contact_matrix.shape)
    percentile_to_discard = 10
    norm_contact_matrix_filtered = discard_lowest_percent(norm_contact_matrix_without_self_loops, percentile_to_discard)
    ...
    print(f"Performing clustering using normalized contacts without self-loops and discarding lowest {percentile_to_discard}% contacts")
    predicted_labels = perform_metacc_leiden_clustering(norm_contact_matrix_filtered, random_seed)
