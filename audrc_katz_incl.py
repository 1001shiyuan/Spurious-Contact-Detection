...
def main(beta=0.01):
    ...
    katz_matrix_raw = katz_index(contact_matrix, beta)
    katz_scores_raw = calculate_scores(contact_matrix, katz_matrix_raw, true_positives)
    audrc_katz_raw, tpr_katz_raw, discard_proportion_katz_raw = calculate_audrc(katz_scores_raw)
    katz_matrix_norm = katz_index(norm_contact_matrix, beta)
    katz_scores_norm = calculate_scores(norm_contact_matrix, katz_matrix_norm, true_positives)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('audrc_katz_incl_self_loops.png')
    plt.show()
