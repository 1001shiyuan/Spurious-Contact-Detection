...
def evaluate_clustering(true_labels, predicted_labels, metadata):
    valid_bins = metadata['Contig length'] > 150000
    true_labels = true_labels[valid_bins]
    predicted_labels = np.array(predicted_labels)[valid_bins]
...
def main():
    ...
    norm_contact_matrix = exclude_self_loops(norm_contact_matrix)  
    ...
    for metric_name, metric_function in similarity_metrics.items():
        ...
        ari, nmi = evaluate_clustering(true_labels, predicted_labels, metadata)
